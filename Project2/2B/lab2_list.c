...
int main(int argc, char *argv[])
{
  	...
  	long average_wait = total_waittime / ((num_iterations * 2 + num_lists) * num_threads);
  	printf("list-%s-%s,%d,%d,%d,%d,%ld,%ld,%ld\n", yieldopts, syncopts[opt_sync], 
  		num_threads, num_iterations, num_lists, performed, runtime, runtime/performed, average_wait);
